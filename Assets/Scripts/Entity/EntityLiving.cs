...
public partial class EntityLiving : Entity {
    ...
    protected CollisionObject3D collisionObject3D;
    ...
    /// 控制动画统一进行更新的节点
    /// </summary>
    protected ScriptPlayable<UpdatePlayableBehaviour> updatePlayableBehaviourScriptPlayable;

    public class UpdatePlayableBehaviour : PlayableBehaviour {
        protected EntityLiving entityLiving;

        public override void PrepareFrame(Playable playable, FrameData info) {
            base.OnBehaviourPlay(playable, info);
            entityLiving.updateAnimation(playable, info);
        }

        public void setEntityLiving(EntityLiving _entityLiving) {
            entityLiving = _entityLiving;
        }
    }
    ...
    /// 实体所携带的所有动画
    /// </summary>
    protected Map<string, AnimationPlayableBehaviour> animationPlayableBehaviourName;

    /// <summary>
    /// 动画混合器
    /// </summary>
    protected AnimationMixerPlayable animationMixerPlayable;

    /// <summary>
    /// 当前动画正在过度
    /// </summary>
    protected bool isExcessive;

    /// <summary>
    /// 过度时间
    /// </summary>
    protected float excessiveTime;

    /// <summary>
    /// 所有的过渡时间
    /// </summary>
    protected float allExcessiveTime;
    ...
    protected override void enterTreeInit() {
        ...
        {
            animationPlayableBehaviourName = new Map<string, AnimationPlayableBehaviour>();
            playableGraph = PlayableGraph.Create();

            updatePlayableBehaviourScriptPlayable = ScriptPlayable<UpdatePlayableBehaviour>.Create(playableGraph);
            updatePlayableBehaviourScriptPlayable.GetBehaviour().setEntityLiving(this);
            updatePlayableBehaviourScriptPlayable.SetInputCount(1);
            updatePlayableBehaviourScriptPlayable.SetOutputCount(1);

            animationMixerPlayable = AnimationMixerPlayable.Create(playableGraph, animationPlayableBehaviourName.Count);
            animationMixerPlayable.SetOutputCount(1);
            playableGraph.Connect(animationMixerPlayable, 0, updatePlayableBehaviourScriptPlayable, 0);

            animationPlayableOutput = AnimationPlayableOutput.Create(playableGraph, main, animator);
            animationPlayableOutput.SetSourcePlayable(updatePlayableBehaviourScriptPlayable);
        }
    }
    ...
    protected override void initEnd() {
        ...
        //initEffect
        {
            ...
            foreach (var entityEffectStack in effectStack) {
                ...
                list.Add(
                    new Event.EventEntity.EventLiving.EventLivingEffect.EventLivingAddEffect.EventLivingInitAddEffect(this, entityEffectStack.Key, entityEffectStack.Value));
            }
            addNextTimeRun(0, () => {
    ...
    public void tryRotation(Vector3 pos, bool isPointTo = false) => tryRotation(Quaternion.LookRotation(isPointTo ? pos : pos - Position).eulerAngles.y);
    ...
    protected void updateCharacterControllerMove() {
        float deltaTime = getDeltaTime(TriggerType.update, TimeType.part);
        Vector3 _move = move.fixedUpdateMove * deltaTime;
        _move += move.move * deltaTime;
        tryMove(_move);

        float rotation = move.targetRotation;
        set(AllEntityState.onRotation, (Quaternion.eulerAngles.y - rotation).isEffective(0.01f) && !moveLimit.rotateLock);
        if (moveLimit.rotateLock) {
            return;
        }
        if (moveLimit.rotationSmoothTime.tryGet(out var rotationSmoothTime)) {
            rotation = Mathf.SmoothDampAngle(transform.eulerAngles.y, move.targetRotation, ref move.rotationVelocity, rotationSmoothTime,
                float.PositiveInfinity, getDeltaTime(ScaleManage.TriggerType.update, ScaleManage.TimeType.part));
        }
        transform.rotation = Quaternion.FromEuler(new Vector3(0.0f, rotation, 0.0f));
    }

    [TimerCellCycle(triggerType = TriggerType.update)]
    protected void updateAnimationExcessive() {
        if (isExcessive) {
            excessiveTime += getDeltaTime(ScaleManage.TriggerType.update, ScaleManage.TimeType.part);
            float weight = excessiveTime / allExcessiveTime;
            weight = math.clamp(weight, 0, 1);
            animationMixerPlayable.SetInputWeight(mainAnimationPlayableBehaviour.getId(), 1 - weight);
            if (excessiveAnimationPlayableBehaviour is not null) {
                animationMixerPlayable.SetInputWeight(excessiveAnimationPlayableBehaviour.getId(), weight);
            }
            if (excessiveTime >= allExcessiveTime) {
                animationMixerPlayable.SetInputWeight(mainAnimationPlayableBehaviour.getId(), 0);
                mainAnimationPlayableBehaviour.pause();
                mainAnimationPlayableBehaviour = excessiveAnimationPlayableBehaviour;
                excessiveAnimationPlayableBehaviour = null;
                if (mainAnimationPlayableBehaviour is not null) {
                    animationMixerPlayable.SetInputWeight(mainAnimationPlayableBehaviour.getId(), 1);
                }
                isExcessive = false;
            }
            return;
        ...
        if (selectedAnimationPlayableBehaviour is not null) {
            if (mainAnimationPlayableBehaviour.getType().HasFlag(AnimationType.death)) {
                new Event.EventEntity.EventLiving.EventLivingDeath.EventLivingDoDeath(this).onEvent();
                onLivingDeath();
                return;
            }
            mainAnimationPlayableBehaviour.end();
            excessiveAnimationPlayableBehaviour = selectedAnimationPlayableBehaviour;
            selectedAnimationPlayableBehaviour = null;
            excessiveAnimationPlayableBehaviour.start();
            excessiveTime = 0;
            allExcessiveTime = excessiveAnimationPlayableBehaviour.getBlendTime();
            isExcessive = true;
        }
    }
    ...
    /// 更新动作
    /// </summary>
    protected virtual void updateAnimation(Playable playable, FrameData info) {
        if (!isPlayer) {
            return;
        }
        mainAnimationPlayableBehaviour.update(playable, info);
        excessiveAnimationPlayableBehaviour?.update(playable, info);
    }
    ...
    protected virtual void fixedUpdateEffect() {
        ...
        foreach (var entityEffectStack in effectStack) {
            entityEffectStack.Value.time -= getDeltaTime(TriggerType.fixedUpdate, entityEffectStack.Key.getEffectTimeType());
            if (entityEffectStack.Value.isEmpty()) {
    ...
    protected virtual void fixedUpdateMoveToward() {
        ...
        //当前动画状态不允许移动时更改状态
        if (getAnimation().getLimitLevel() > 0 || has(AllEntityState.inAir)) {
    ...
    protected virtual void fixedUpdateStandState() {
        ...
        if (move.move.Y > _groundedDetection.groundedRadius) {
        ...
        spherePosition.y += _groundedDetection.groundedOffset;
        bool isGrounded = Physics.CheckSphere(spherePosition, _groundedDetection.groundedRadius, layerMask, QueryTriggerInteraction.Ignore);
    ...
    public virtual void fixedUpdateAirAnimation() {
        ...
        if (!canSet(airAnimationPlayableBehaviour)) {
            if (!getAnimation().getType().HasFlag(AnimationType.air)) {
                return;
            }
        }
    ...
    protected virtual void fixedUpdateAnimation() {
        mainAnimationPlayableBehaviour.fixedUpdate();
        if (excessiveAnimationPlayableBehaviour is not null) {
            excessiveAnimationPlayableBehaviour.fixedUpdate();
        }
    }
    ...
    public void set(AnimationPlayableBehaviour _animation, bool replay = false) {
        ...
        if (mainAnimationPlayableBehaviour is null) {
            ...
            excessiveAnimationPlayableBehaviour = null;
            animationMixerPlayable.SetInputWeight(mainAnimationPlayableBehaviour.getId(), 1);
            mainAnimationPlayableBehaviour.start();
            return;
        }
        ...
        selectedAnimationPlayableBehaviour?.recovery();
    }
    ...
    public bool canSet(
        [CanBeNull] AnimationPlayableBehaviour _animation,
        ...
        if (has(AllEntityState.beVoid)) {
            ...
            if (
                _animation.getType().HasFlag(AnimationType.attack)
                || _animation.getType().HasFlag(AnimationType.skill)
                || _animation.getType().HasFlag(AnimationType.dodge)
            ) {
                return false;
            }
        }
    ...
    /// 获取当前的动画
    /// </summary>
    public AnimationPlayableBehaviour getAnimation() => isExcessive ? excessiveAnimationPlayableBehaviour : mainAnimationPlayableBehaviour;

    protected void addAnimation(AnimationPlayableBehaviour animationPlayableBehaviour) {
        if (animationPlayableBehaviourName.ContainsKey(animationPlayableBehaviour.getName())) {
            throw new Exception();
        }
        animationPlayableBehaviour.setId(animationPlayableBehaviourName.Count);
        animationMixerPlayable.SetInputCount(animationPlayableBehaviourName.Count + 1);
        animationPlayableBehaviour.init(this, playableGraph, animationMixerPlayable);
        animationPlayableBehaviourName.Add(animationPlayableBehaviour.getName(), animationPlayableBehaviour);
        animationPlayableBehaviour.pause();
    }

    /// <summary>
    /// 判断实体是不是死亡
    /// </summary> 
    public bool isDeath() => getAnimation().getType().HasFlag(AnimationType.death);
    ...
    public void recoverDefaultMaterial() {
        ...
        skinnedMeshRenderer.materials = defaultMaterial;
    }
    ...
    protected virtual void onEvent_blowUpRecovery(Event.EventEntity.EventLiving.EventLivingState.EventLivingEndState @event) {
        if (!@event.entityState.Equals(AllEntityState.inAir)) {
            return;
        }
        if (!getAnimation().Equals(blowUpAnimationPlayableBehaviour)) {
            return;
        }
    ...
    protected virtual void onEvent_defaultNextAnimation(Event.EventEntity.EventLiving.EventLivingAnimator.EventSettlementNextAnimation @event) {
        if (!@event.animationPlayableBehaviour.getLimitType().Equals(AnimationLimitType.end)) {
            return;
        }
    ...
    protected virtual void onEvent_switchAttackSpeed(Event.EventEntity.EventLiving.EventLivingAnimator.EventLivingStartAnimation @event) {
        if (!@event.animationPlayableBehaviour.getType().HasFlag(AnimationType.attack)) {
            return;
        }
        @event.animationPlayableBehaviour.setSeepMultiple((float)get(AllAttribute.attackSpeed));
    }
    ...
    protected virtual void onEvent_switchCastSpeed(Event.EventEntity.EventLiving.EventLivingAnimator.EventLivingStartAnimation @event) {
        if (!@event.animationPlayableBehaviour.getType().HasFlag(AnimationType.skill)) {
            return;
        }
        @event.animationPlayableBehaviour.setSeepMultiple((float)get(AllAttribute.spellSpeed));
    }
    ...
    protected virtual void onEvent_switchMoveSpeed(Event.EventEntity.EventLiving.EventLivingAnimator.EventLivingStartAnimation @event) {
        if (!@event.animationPlayableBehaviour.getType().HasFlag(AnimationType.move)) {
            return;
        }
        @event.animationPlayableBehaviour.setSeepMultiple((float)get(AllAttribute.speed));
    }
    ...
    protected virtual void onEvent_switchMoveSpeed(Event.EventEntity.EventLiving.EventAttribute.EventCatchAttribute @event) {
        if (!getAnimation().getType().HasFlag(AnimationType.move)) {
            return;
        }
        double newSeep = @event.map.get(AllAttribute.speed).getModificationValue();
        if (Math.Abs(newSeep - get(AllAttribute.speed)) < 0.01) {
            return;
        }
        getAnimation().setSeepMultiple((float)newSeep);
    }
    ...
    protected virtual void onEvent_addTimeEffect(Event.EventEntity.EventLiving.EventLivingEffect.EventLivingAddEffect @event) {
        if (!@event.effect.Equals(entityEffectTimeScale)) {
            return;
        }
    ...
    public static bool canPlayStatic(AnimationPlayableBehaviour old, AnimationPlayableBehaviour? @new, AnimationPlayDetermineType animationPlayDetermineType) {
        int newL = @new?.getLimitLevel() ?? 0;
        int oldL = old.getLimitLevel();
        if (newL == 0 && oldL == 0) {
            return true;
        }
        bool isBackSway = old.getLimitType() >= backSway;
    ...
    public class MoveLimit {
        ...
        /// 旋转平滑时间
        /// </summary>
        public ControlledField<float> rotationSmoothTime = 0.12f;
...
/// </summary>
[Serializable]
public class AnimationPlayableBehaviour : IFieldSupply<AnimationPlayableBehaviour> {
    protected EntityLiving entityLiving;
    protected PlayableGraph playableGraph;
    ...
    /// 基础速度
    /// </summary>
    [SerializeField] protected ControlledField<float> seepBasics;

    /// <summary>
    /// 混合时间
    /// </summary>
    [SerializeField] protected ControlledField<float> blendTime = 0.1f;

    /// <summary>
    /// 重播时间 
    /// </summary>
    [SerializeField] protected ControlledField<float> _replayTime;

    /// <summary>
    /// 前摇结束时间
    /// </summary>
    [SerializeField] protected ControlledField<float> forwardTime;

    /// <summary>
    /// 后摇开始时间
    /// </summary>
    [SerializeField] protected ControlledField<float> backTime;

    protected List<DataStruct<float, Action>> posAction = new List<DataStruct<float, Action>>();
    ...
    /// 限制类型，前后摇
    /// </summary>
    protected AnimationLimitType animationLimitType;
    ...
    /// 如果是单一动画将直接输出
    /// </summary>
    protected AnimationClipPlayable animationClipPlayable;

    /// <summary>
    /// 重播
    /// </summary>
    protected AnimationClipPlayable supersedeAnimationClipPlayable;
    ...
    public virtual void init(EntityLiving _entityLiving, PlayableGraph _playableGraph, AnimationMixerPlayable _animationMixerPlayable) {
        this.entityLiving = _entityLiving;
        this.playableGraph = _playableGraph;

        //triggerEventList = new ArrayList<EventAtomPos>(triggerEventList.OrderBy(t => t.getPos()));
        if (animationClip == null) {
            animationClip = AllAnimationClip.empty;
        }
        maxTime = animationClip.length;
        animationClipPlayable = AnimationClipPlayable.Create(playableGraph, animationClip);
        animationClipPlayable.SetOutputCount(1);
        animationClipPlayable.Pause();
        if (_replayTime.tryGet(out _)) {
            supersedeAnimationClipPlayable = AnimationClipPlayable.Create(playableGraph, animationClip);
            animationMixerPlayable = AnimationMixerPlayable.Create(playableGraph);
            animationMixerPlayable.SetInputCount(2);
            animationClipPlayable.SetOutputCount(1);
            playableGraph.Connect(animationMixerPlayable, 0, _animationMixerPlayable, id);
            playableGraph.Connect(animationClipPlayable, 0, animationMixerPlayable, 0);
            playableGraph.Connect(supersedeAnimationClipPlayable, 0, animationMixerPlayable, 1);
            animationMixerPlayable.SetInputWeight(0, 1);
            supersedeAnimationClipPlayable.Pause();
        }
        else {
            playableGraph.Connect(animationClipPlayable, 0, _animationMixerPlayable, id);
        }
    }

    public void init([CanBeNull] AnimationPlayableBehaviourData animationPlayableBehaviourData, string _name) {
        if (animationPlayableBehaviourData is null) {
            return;
        }
        animationType = animationPlayableBehaviourData.animationType;
        limitLevel = animationPlayableBehaviourData.limitLevel;
        name = _name;
    }

    public IEnumerable<AnimationPlayableBehaviour> supply(FieldInfo fieldInfo) {
        init(fieldInfo.GetCustomAttribute<AnimationPlayableBehaviourData>(), fieldInfo.Name);
        name = fieldInfo.Name;
        yield return this;
    }
    ...
    public virtual void start() {
        ...
        nextTriggerId();
        /*if (!effectCell.isEmpty()) {
            for (int i = 0; i < effectCell.Count; i++) {
                effectCell[i].create(entityLiving, $"animation.effect.{name}.{i}");
            }
        }*/
        new Event.EventEntity.EventLiving.EventLivingAnimator.EventLivingStartAnimation(entityLiving, this).onEvent();
    }
    ...
    public virtual void replay() {
        ...
        time = 0;
        isReplay = true;
        animationMixerPlayable.SetInputWeight((replayTag ? animationClipPlayable : supersedeAnimationClipPlayable), 0);
        (replayTag ? animationClipPlayable : supersedeAnimationClipPlayable).SetTime(0);
    }
    ...
    public virtual void end() {
        ...
        /*if (!effectCell.isEmpty()) {
            for (int i = 0; i < effectCell.Count; i++) {
                entityLiving.clearSonEntity($"animation.effect.{name}.{i}");
            }
        }*/
        new Event.EventEntity.EventLiving.EventLivingAnimator.EventLivingEndAnimation(entityLiving, this).onEvent();
    }

    /// <summary>
    /// 播放
    /// </summary>
    public virtual void play() => isPlay = true;

    /// <summary>
    /// 暂停
    /// </summary>
    public virtual void pause() => isPlay = false;

    public virtual void recovery() {
        animationLimitType = forwardTime.isAdopt() ? AnimationLimitType.forwardSway : AnimationLimitType.normal;
        time = 0;
        loopCount = 0;
        triggerId = -1;
        seep = seepBasics.tryGet(out var s) ? s : 1;
        isReplay = false;
        isPlay = false;
        excessiveTime = 0;
        if (_replayTime.isAdopt()) {
            animationClipPlayable.SetTime(0);
            supersedeAnimationClipPlayable.SetTime(0);
        }
        else {
            animationClipPlayable.SetTime(0);
        }
    ...
    public virtual void update(Playable playable, FrameData info) {
        if (isReplay) {
            //excessiveTime += entityLiving.getDeltaTime(ScaleManage.TriggerType.update, ScaleManage.TimeType.part);
            excessiveTime += info.deltaTime * entityLiving.getTimeScale() * Time.timeScale;
            float weight = excessiveTime / _replayTime.forceGet();
            weight = math.clamp(weight, 0, 1);
            {
                animationMixerPlayable.SetInputWeight(replayTag ? supersedeAnimationClipPlayable : animationClipPlayable, 1 - weight);
                (replayTag ? supersedeAnimationClipPlayable : animationClipPlayable).SetTime(time);
            }
            {
                animationMixerPlayable.SetInputWeight(replayTag ? animationClipPlayable : supersedeAnimationClipPlayable, weight);
                (replayTag ? animationClipPlayable : supersedeAnimationClipPlayable).SetTime(excessiveTime);
            }
            if (excessiveTime >= _replayTime.forceGet()) {
                excessiveTime = 0;
                isReplay = false;
                animationMixerPlayable.SetInputWeight(replayTag ? supersedeAnimationClipPlayable : animationClipPlayable, 0);
                (replayTag ? supersedeAnimationClipPlayable : animationClipPlayable).SetTime(0);
                replayTag = !replayTag;
            }
        }

        if (isPlay) {
            //time += seep * entityLiving.getDeltaTime(ScaleManage.TriggerType.update, ScaleManage.TimeType.part);
            time += seep * info.deltaTime * entityLiving.getTimeScale() * Time.timeScale;
        }
        ...
        switch (animationLimitType) {
            case AnimationLimitType.forwardSway:
                if (forwardTime.tryGet(out var f) && time > f) {
                    animationLimitType = AnimationLimitType.normal;
                }
                break;
            case AnimationLimitType.normal:
                if (backTime.tryGet(out var b) && time > b) {
                    animationLimitType = AnimationLimitType.backSway;
                    entityLiving.nextAnimation(this);
                }
                break;
        }

        if (time > maxTime && animationLimitType != AnimationLimitType.end) {
            if (getType().HasFlag(AnimationType.loop)) {
                triggerId = -1;
                loopCount++;
                nextTriggerId();
                return;
            }
            animationLimitType = AnimationLimitType.end;
            entityLiving.nextAnimation(this);
        }
    }
    ...
    public virtual void fixedUpdate() {
        /*if (triggerId >= 0) {
            EventAtomPos triggerPos = triggerEventList[triggerId];
            if (time > triggerPos.getPos() + loopCount * maxTime) {
                triggerEvent(triggerPos);
                nextTriggerId();
            }
        }*/
        if (triggerId >= 0) {
            DataStruct<float, Action> triggerPos = posAction[triggerId];
            if (time > triggerPos.k + loopCount * maxTime) {
                triggerPos.v();
                nextTriggerId();
            }
        }
    ...
    public virtual bool isUse() => use;
    public virtual int getLimitLevel() => limitLevel;

    public virtual AnimationType getType() => animationType;

    public virtual AnimationLimitType getLimitType() => animationLimitType;
    ...
    public float getBlendTime() => blendTime.tryGet(out var b) ? b : 0;

    public void nextTriggerId() {
        triggerId++;
        /*if (triggerId >= triggerEventList.Count) {
            triggerId = -1;
        }*/
        if (triggerId >= posAction.Count) {
            triggerId = -1;
        }
    }

    public void setSeepMultiple(float multiple) {
        seep = multiple * (seepBasics.tryGet(out var s) ? s : 1);
    }

    public override string ToString() {
        return $"{nameof(id)}: {id}, {nameof(name)}: {name}, {nameof(animationClip)}: {animationClip}";
    }
