...
public partial class Entity : Node3D {
    ...
    /// 作为子实体的名字
    /// </summary>
    protected string workSonEntityName;

    /// <summary>
    /// 运行时的子实体
    /// </summary>
    protected Dictionary<string, Entity> runTimeSonEntity = new Dictionary<string, Entity>();

    /// <summary>
    /// 计时运行
    /// </summary>
    protected TimeRun timeRun;
    ...
    /// 在初始化时生成对应实例的事件发布器
    /// </summary>
    protected IEventBus eventsRunPackGather;
    ...
    protected void enterTreeNecessary() {
        ...
        eventsRunPackGather = new EventBus.EventBus();
    ...
    protected virtual void enterTreeInitMethod(MethodInfo methodInfo) {
        ...
        if (timerCellCycleAttribute is not null) {
            ...
            addCycleRun(timerCellCycleAttribute.timer, @delegate as Action, true, timerCellCycleAttribute.priority, timerCellCycleAttribute.triggerType,
    ...
    /// 添加前请用has(_name)检查
    /// </summary>
    public virtual void addSonEntity(string entityName, Entity entity, Node node = null, bool worldPositionStays = false) {
        node ??= this;
        node.Reparent(entity, worldPositionStays);
    ...
    /// 用实体id当做key
    /// </summary>
    public void addSonEntity(Entity entity, Node _transform = null, bool worldPositionStays = false) =>
